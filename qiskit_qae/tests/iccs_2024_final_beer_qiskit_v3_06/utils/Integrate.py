...
def qae_wind_to_dict(wind_ts, start, step):
    wind_set = {}
...
def qae_seq_1_plot(in_org_seq, marker_points=None, markers_on_top=True, vert_line=True, line_offset=2,
    ...
    for sel_wind in sorted_keys:
        ...
        if marker_points is not None and not markers_on_top:
            ...
            plt.plot(qae_seq_x_range(sel_wind, markers), markers, marker='.', color=marker_cols[sub_seq], linestyle='None')
        
        # Plot target function
        plt.plot(qae_seq_x_range(sel_wind, org_in), org_in, color=cols[sub_seq], label=labels[sub_seq])
...
def qae_seq_n_plot(
    ...
    for i in range(len(y_train_noisy_ls)):
        plt.plot(list(sel_train_range), y_train_noisy_ls[i], color=cols['train_noisy'], marker='.', linestyle=line_styles['train_noisy'])
        
    for i in range(len(y_valid_noisy_ls)):
        plt.plot(list(sel_valid_range), y_valid_noisy_ls[i], color=cols['valid_noisy'], marker='.', linestyle=line_styles['valid_noisy'])
...
def qae_seq_plot(in_org_seq, in_meas_seq, out_recons_seq, 
    ...
    for sel_wind in sorted_keys:
        ...
        # Plot target function
        plt.plot(qae_seq_x_range(sel_wind, org_in), org_in, linestyle=line_styles[0], color=line_cols[0])
        if add_markers:
            plt.plot(qae_seq_x_range(sel_wind, org_in), org_in, marker='o', color=marker_line_cols[0], linestyle=marker_line_style)
        plt.plot(qae_seq_x_range(sel_wind, org_meas), org_meas, linestyle=line_styles[1], color=line_cols[1])
        if add_markers:
            plt.plot(qae_seq_x_range(sel_wind, org_meas), org_meas, marker='o', color=marker_line_cols[1], linestyle=marker_line_style)
        plt.plot(qae_seq_x_range(sel_wind, out_meas), out_meas, linestyle=line_styles[2], color=line_cols[2])
        if add_markers:
            plt.plot(qae_seq_x_range(sel_wind, out_meas), out_meas, marker='o', color=marker_line_cols[2], linestyle=marker_line_style)
...
def qae_diffaug_plot(y_train_enc, y_valid_enc, y_train_noisy_enc, y_valid_noisy_enc, wind_step, noise, noise_reps,
                     labels=['Training', 'Validation'], xlabel='Range', ylabel='Target value (deltas)', 
                     line_styles={'train_pure':'solid', 'train_noisy':'none', 'valid_pure':'solid', 'valid_noisy':'none'},
                     cols = {'train_pure':'blue', 'train_noisy':'lightblue', 'valid_pure':'red', 'valid_noisy':'pink'},
                     xlim=None, ylim=None, title=None, save_plot=None, sel_wind=None):
    ...
    qae_seq_n_plot(train_pure_vals, valid_pure_vals, y_train_noisy_flats, y_valid_noisy_flats,
                   title=title, xlabel=xlabel, ylabel=ylabel, xlim=xlim, ylim=ylim,
                   cols=cols, line_styles=line_styles, save_plot=save_plot)
